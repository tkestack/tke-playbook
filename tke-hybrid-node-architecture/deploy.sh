...
echo "请输入可用区列表，用逗号分隔（默认: ap-singapore-1,ap-singapore-2,ap-singapore-3）: "
read -p "> " AVAILABILITY_ZONES
AVAILABILITY_ZONES=${AVAILABILITY_ZONES:-"ap-singapore-1,ap-singapore-2,ap-singapore-3"}
...
# 将可用区字符串转换为Terraform列表格式
FORMATTED_AZS="[\"$(echo $AVAILABILITY_ZONES | sed 's/,/\",\"/g')\"]"
...
terraform apply -auto-approve \
  ...
  -var="availability_zones=$FORMATTED_AZS" \
